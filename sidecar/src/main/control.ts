...
//     display. Ghost drives the SAME `DISPLAY` (Core passes it through the MCP env
//     allowlist), so agents and the human see/control one screen.
...
export function resolveControlPort(
	...
	const explicit = Number.parseInt(env.RYU_DESKTOP_PORT ?? "", 10);
	if (Number.isInteger(explicit) && explicit > 0) {
		return explicit;
	}
	const isDev = (env.RYU_PROFILE ?? "").trim().toLowerCase() === "dev";
	return isDev
		? DESKTOP_CONTROL_BASE_PORT + DEV_PORT_OFFSET
		: DESKTOP_CONTROL_BASE_PORT;
}

export function resolveControlToken(
	env: NodeJS.ProcessEnv = process.env
): string | null {
	const raw = env.RYU_EXT_TOKEN ?? env.RYU_DESKTOP_TOKEN ?? "";
...
const ok = (json?: unknown): ControlResponse => ({ json: json ?? { ok: true }, status: 200 });
const notFound = (): ControlResponse => ({ json: { error: "not found" }, status: 404 });
...
export async function handleRequest(
	...
	if (req.method === "POST" && pathname === "/input") {
		const body = (await readJsonBody(req)) as { type?: string; x?: number; y?: number; text?: string; key?: string; buttons?: number } | undefined;
...
export function startControlServer(
	...
	const server = createServer(async (req, res) => {
		...
		try {
			const resp = await handleRequest(controller, token, req, url);
	...
	server.on("upgrade", (req, socket, head) => {
		const url = new URL(req.url ?? "/", "http://127.0.0.1");
		if (url.pathname === "/ws" && wsAllowed(req.headers, token)) {
