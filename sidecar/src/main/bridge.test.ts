...
import { LinuxDesktopController } from "./desktop.ts";
...
class ReadyStubController {
	...
	teardown(): void {}
	capabilities() {
		return { available: true, display: ":99", vncPort: this.port, resolution: "1600x1000x24" };
	}
...
describe("ryu-desktop websockify bridge", () => {
	...
	beforeAll(async () => {
		...
		const controller = new ReadyStubController(vnc.port) as unknown as Parameters<
			typeof startControlServer
		>[0];
	...
	test("WS → TCP → WS round-trips RFB bytes through the bridge", async () => {
		...
		const sent = Buffer.from([0x52, 0x46, 0x42, 0x20, 0x30, 0x30, 0x33, 0x2e, 0x30, 0x30, 0x38, 0x0a]);
