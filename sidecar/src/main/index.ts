...
// the selected Bot's screen. No window, no Electron — the desktop is a headless X
// session.
...
import { DesktopSessionManager } from "./desktop.ts";
...
function main(): void {
	...
	// Preserve the original eager default screen while additional Bot sessions are
	// started lazily after Core allocates their displays. Best-effort: a missing
	// toolchain reports unavailable, never crashes the sidecar.
	void sessions.controllerFor("ryu").then((controller) => {
		const r = controller.capabilities().available
			? { ok: true as const }
			: { ok: false as const, error: controller.unavailableReason() };
		if (r.ok) {
			console.log(
	...
	const bridge = createWsBridge(sessions);
	startControlServer(sessions, token, port, (req, socket, head) =>
